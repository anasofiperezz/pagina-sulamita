...
async function readJson(filename, fallback = []) {
  try {
    ...
    console.warn(
      `No se pudo leer ${filename}. Se utilizará un arreglo vacío.`
    );
...
async function updateTableSequence(client, tableName) {
  await client.query(
    `
      SELECT setval(
...
async function migrateUsuarios(client) {
  ...
  await updateTableSequence(client, "usuarios");
}
...
async function migrateProductos(client) {
  ...
  await updateTableSequence(client, "productos");
}
...
async function migratePedidos(client) {
  ...
  await updateTableSequence(client, "pedidos");
}
...
async function migrateContactos(client) {
  ...
  await updateTableSequence(client, "contactos");
}
...
async function migrate() {
  ...
  try {
    console.log(
      "Iniciando migración de archivos JSON a PostgreSQL..."
    );
